...
enum nvme_nvm_admin_opcode {
	nvme_nvm_admin_identity		= 0xe2,  //figure 9 NVMe Opcode
	nvme_nvm_admin_get_bb_tbl	= 0xf2,
	nvme_nvm_admin_set_bb_tbl	= 0xf1,
};

enum nvme_nvm_log_page {
	NVME_NVM_LOG_REPORT_CHUNK	= 0xca,  //Figure 13 Log Identifier (opcode)
};
...
/*
 * Check we didn't inadvertently不经意地 grow the command struct
 */
				 ...
				 struct nvme_nvm_id20_addrf *src)
{
	...
	dst->ch_mask = ((1ULL << dst->ch_len) - 1) << dst->ch_offset;    //((1ULL << dst->ch_len) - 1)使64位中的末ch_len位全为1，其余位为0；<< dst->ch_offset左移offset使其对齐
	dst->lun_mask = ((1ULL << dst->lun_len) - 1) << dst->lun_offset;
	dst->chk_mask = ((1ULL << dst->chk_len) - 1) << dst->chk_offset;
	dst->sec_mask = ((1ULL << dst->sec_len) - 1) << dst->sec_offset;
}

//get related parameters (nvme_nvm_id20 *id →传入→ nvm_geo *geo)
...
//Device Geometry Command
//首先将Lightnvm request转换成NVMe command，然后直接调用NVMe模块提供的nvme_submit_sync_cmd函数（在这其中初始化了一个新的request），然后执行这个request，通过nvme_nvm_setup_20函数获取设备几何结构信息
...
//General IO
//首先构造NVMe Command，然后根据command和bio初始化生成request，最后调用块设备层的blk_execute_rq_nowait()函数执行请求
